...
class TestProductMethods(unittest.TestCase):
	...
	def test4_delete(self):
		...
		product_id  = products.create_product(name='test_creating_prod_GooeeIOT', api_key=API_KEY).json()['id']
		count += 1
		status_code = products.delete_product(product_id=str(product_id), api_key=API_KEY).status_code
		self.assertEqual(status_code, 200)
		self.assertEqual(len(products.list_products(api_key=API_KEY).json()), count - 1)
...
class TestActionMethods(unittest.TestCase):
	...
	def test1_(self):
		...
		#creating necessary objects required for making test action: thng || product || shortID
		product_id = products.create_product(name='test_creating_prod_GooeeIOT', api_key=API_KEY).json()['id']
		...
		action_id = actions.create_action(type_='scans', #need to create more on every base action types.
										api_key=API_KEY,
										customFields={'test_custom_Field': 'test_value'}, 
										product = str(product_id)).json()['id']		
		#checking if status code == 200 
		status_code = actions.delete_action(type_='scans', action_id=str(action_id), api_key=API_KEY).status_code #why we need type when we know action id?
		self.assertEqual(status_code, 200)
		products.delete_product(product_id=str(product_id), api_key=API_KEY)

	def test2_list_actions(self):
		count = len(actions.list_actions(type_='scans',api_key=API_KEY).json()) #save number of actions before we create new one.
		...
		action_id = actions.create_action(type_='scans',
										...
										customFields={'test_custom_Field': 'test_value'}, 
										product = str(product_id)).json()['id']
		self.assertEqual(len(actions.list_actions(type_='scans', api_key=API_KEY).json()), count + 1)

		#delete test objects
		actions.delete_action(type_='scans', action_id=str(action_id), api_key=API_KEY)
		products.delete_product(product_id=str(product_id), api_key=API_KEY)
